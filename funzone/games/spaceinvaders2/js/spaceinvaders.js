...
PlayState.prototype.enter = function(game) {
  ...
  this.invaderInitialVelocity =
    ...
    levelMultiplier * this.config.invaderInitialVelocity
...
var saveScore = function(score) {
  ...
  if (user) {
    firestore
      ...
      .then(function(doc) {
        if (
          doc.data().spaceinvadersscore === undefined ||
          doc.data().spaceinvadersscore < score
        ) {
          firestore
            .collection('users')
            .doc(user.uid)
    ...
    firestore
      ...
      .then(function(doc) {
        if (doc.data() === undefined) {
          ...
          if (doc.data().spaceinvadersscore < score) {
            firestore
              .collection('highscores')
              .doc('spaceinvaders')
              .update({
...
PlayState.prototype.draw = function(game, dt, ctx) {
  ...
  //  Draw info.
  var textYpos =
    game.gameBounds.bottom + (game.height - game.gameBounds.bottom) / 2 + 14 / 2
  ctx.font = '14px Arial'
  ctx.fillStyle = '#ffffff'
...
PlayState.prototype.fireRocket = function() {
  ...
  if (
    ...
    new Date().valueOf() - this.lastRocketTime >
      1000 / this.config.rocketMaxFireRate
  ) {
    //  Add a rocket.
    this.rockets.push(
